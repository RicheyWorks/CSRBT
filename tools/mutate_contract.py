...
MUTANTS += [
    ...
    ("the manifest does not say how deep the ring is",
     ...
     "the manifest says the session exists"),
    # ---- ADR-231: if_stamp guards either document -----------------------------
    ("a report-stamped guard looks and ignores the answer: it never refuses stale",
     '            if now != want:\n                raise Stale("%s\'s REPORT',
     '            if False:\n                raise Stale("%s\'s REPORT',
     "the figures have moved past was run"),
    ("a target with no report series is compared anyway, so the refusal says the page moved",
     "            if now is None:\n                raise InvalidArgument(",
     "            if False:\n                raise InvalidArgument(",
     "is the wrong KIND"),
    ("a string that is not a stamp of either series is compared to the snapshot and refused stale",
     "        if kind is None:\n            # Not a stamp of either series.",
     "        if False:\n            # Not a stamp of either series.",
     "is not a stamp of either series"),
    ("the report refusal sends the caller to observe, which cannot show which figure moved",
     '"nothing was run -- read-report with since=%s to see which figures "',
     '"nothing was run -- observe with since=%s to see which figures "',
     "how to see which figures changed"),
    ("after the report guard passes, the snapshot is compared to the report stamp too, so it never runs",
     '(plugin_id, want, now, want))\n            return\n',
     '(plugin_id, want, now, want))\n            pass\n',
     "A REPORT STAMP IS A GUARD NOW"),
    ("the base plugin answers a report stamp for every target, so a fixture is refused stale not by kind",
     'it can never equal."""\n        return None\n',
     'it can never equal."""\n        return "r" + "0" * 12\n',
     "is the wrong KIND"),
    ("the manifest says the guard takes only the snapshot",
     '"ifStampTakes": ["snapshot", "report"],',
     '"ifStampTakes": ["snapshot"],',
     "the guard takes either kind"),
    ("the trace never records the guard a call carried",
     '        extra = {"guard": guard} if guard else {}',
     '        extra = {}',
     "THE TRACE RECORDS THE GUARD"),
    ("every trace row carries a guard key, empty or not, so older traces no longer read the same",
     '        extra = {"guard": guard} if guard else {}',
     '        extra = {"guard": guard}',
     "THE TRACE RECORDS THE GUARD"),
    ("a refused call's row drops the guard that earned the refusal",
     '                                           "requestId": rid}}, **extra))',
     '                                           "requestId": rid}}))',
     "THE TRACE RECORDS THE GUARD"),
]
