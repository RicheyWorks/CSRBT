...
MUTANTS += [
    ...
    ("the prose is handed over without the thing it sits in",
     ...
     '    .map(e => ({ t: norm(sans(e).textContent).slice(0, 400), host: null }))',
...
MUTANTS += [
    ...
    ("a report whose stamp did not move is sent anyway, as an empty diff",
     ...
     '        if False:\n            return True, "nothing in the report has changed", {',
     "does NOT send the report"),
    ("a report served is never put on the ring, so only the first report is ever a baseline",
    ...
    ("a stamp this session never issued is diffed against whatever is nearest",
     ...
     '        base = (next(reversed(self._reports.values())) if self._reports else None) if since else None',
    ...
    ("the report gets a digest of its own instead of the contract's stamp -- one that knows no noise, so the clock is in it",
     '        st = stamp_of(r, self._report_spec(r), series="r")',
     '        st = "r" + __import__("hashlib").sha256(\n            repr(sorted(r.items())).encode("utf-8")).hexdigest()[:12]',
     "THE CLOCK ROLLING OVER IS NOT THE REPORT MOVING"),
    ...
    ("the report's stamp is in the snapshot's series again, so the two cannot be told apart",
     '        st = stamp_of(r, self._report_spec(r), series="r")',
     '        st = stamp_of(r, self._report_spec(r), series="s")',
    ...
    # ---- ADR-230: the report baseline is a ring -------------------------------
    ("the report ring is one deep again: a stamp two reads old is unknown",
     "REPORT_RING = 8\n",
     "REPORT_RING = 1\n",
     "A STAMP TWO READS OLD IS STILL A BASELINE"),
    ("the report ring is never trimmed",
     "        while len(self._reports) > REPORT_RING:\n            self._reports.popitem(last=False)",
     "        while False:\n            self._reports.popitem(last=False)",
     "holds exactly"),
    # ---- ADR-231: the plugin says what its report's stamp is now --------------
    ("the plugin answers a stamp for every series, so a snapshot guard is compared to a report",
     '        if series != "report":\n            return None\n        self._ensure_settled()',
     '        if series != "report":\n            return "r" + "0" * 12\n        self._ensure_settled()',
     "serves only the report as a second series"),
    ("the guard's look is stamped as a snapshot, so it never equals the report stamp the caller holds",
     '        r = self.page.evaluate(REPORT)\n        return stamp_of(r, self._report_spec(r), series="r")',
     '        r = self.page.evaluate(REPORT)\n        return stamp_of(r, self._report_spec(r), series="s")',
     "what its report's stamp is NOW"),
    ("the guard's look is put on the report ring, so a baseline the caller never read pushes one it did off",
     '        r = self.page.evaluate(REPORT)\n        return stamp_of(r, self._report_spec(r), series="r")',
     '        r = self.page.evaluate(REPORT)\n        st = stamp_of(r, self._report_spec(r), series="r")\n        self._reports[st] = r\n        return st',
     "THE GUARD'S LOOK IS NOT PUT ON THE RING"),
    # ---- ADR-231: the screen chrome does not move the report ------------------
    ("the report names no chrome, so the clock in the keep strip moves the stamp",
     "    if (own) chrome.push(e.id);",
     "    if (false) chrome.push(e.id);",
     "NAMES its screen chrome"),
    ("the chrome is named and then compared anyway: the noise list is never filled",
     '            noise += ["boxes/" + i, "lines/" + i]',
     '            pass',
     "THE CLOCK ROLLING OVER IS NOT THE REPORT MOVING"),
    ("`shown` is stamped, so a pane opening -- or a toast coming into view -- moves the report",
     '                       "noise": ["route", "shown"]}',
     '                       "noise": ["route"]}',
     "without moving the stamp: it is served, and it is noise"),
    ("a box that contains the keep strip carries the clock in its text",
     "    const src = own ? e : sans(e);",
     "    const src = e;",
     "A BOX THAT CONTAINS CHROME IS READ WITHOUT IT"),
    ("`route` is stamped, so a pane switch moves the report",
     '                       "noise": ["route", "shown"]}',
     '                       "noise": ["shown"]}',
     "A PANE SWITCH IS NOT THE REPORT MOVING"),
    ("the report's own noise is dropped when the chrome is added, so route is compared again",
