...
MUTANTS = [
    ...
    ("every kind is a control that hands something over, so a text box is pressed as an export",
     '        if c.get("kind") not in PRESSED:\n            continue',
     '        if False:\n            continue',
     "TEXT BOX whose label happens to say save"),
    # ---- what a button IS (ADR-208) ----
    ("the rule goes back to how a kind is SPELT, so every composed control is pressed",
     '        if c.get("kind") not in PRESSED:\n            continue',
     '        if c.get("kind", "").endswith("_in") or c.get("kind") in ("pick_search",):\n'
     '            continue',
     "NOT ONE OF THEM IS A CANDIDATE"),
    ("what a button is gets written down here instead of read from the door's own pool",
     'PRESSED = frozenset(PP.POOL_KINDS["activate"])',
     'PRESSED = frozenset(["action_btn", "chip", "kopt"])',
     "read from the door's own activate pool"),
    ("the pool is read but emptied, so nothing is a button at all",
     'PRESSED = frozenset(PP.POOL_KINDS["activate"])',
     'PRESSED = frozenset()',
     "is a candidate"),
    # ---- a silent button is not an output (ADR-208) ----
    ("silence is a number in a column again: nothing is ever named",
     '    return [b["key"] for b in r.get("buttons", [])\n'
     '            if b.get("verdict") == "silent" and b["key"] not in declared]',
     '    return []',
     "GREW A BUTTON HANDING NOTHING OVER FAILS"),
    ("the mute worklist carries the emitting buttons instead of the silent ones",
     '            if b.get("verdict") == "silent" and b["key"] not in declared]',
     '            if b.get("verdict") != "silent" and b["key"] not in declared]',
     "hands NOTHING over is named"),
    ("a declared silent button stays on the worklist, so the judgement counts for nothing",
     '            if b.get("verdict") == "silent" and b["key"] not in declared]',
     '            if b.get("verdict") == "silent"]',
     "leaves the mute worklist"),
    ("the mute ceiling is recorded and never enforced",
     '        if mceil is not None and len(qui) > mceil:\n            loud.append((name, len(qui), mceil, qui))',
     '        if False:\n            loud.append((name, len(qui), mceil, qui))',
     "GREW A BUTTON HANDING NOTHING OVER FAILS"),
    ("the mute ratchet runs upward, so a page that grew one records it as the new normal",
     '        if a.raise_floors and (mceil is None or len(qui) < mceil):',
     '        if a.raise_floors and (mceil is None or len(qui) > mceil):',
     "--raise-floors LOWERS the mute ceiling"),
    ("the ledger counts the silent buttons without naming one",
    ...
    ("a risen mute ceiling is printed and the audit still exits zero",
     ...
     "GREW A BUTTON HANDING NOTHING OVER FAILS"),
    ("the mute failure names no page and no button",
     '            print("    %-30s %d silent, ceiling %d: %s"\n'
     '                  % (name, now, mceil, ", ".join(keys[:a.names])))',
     '            print("    a page is over its mute ceiling")',
     "NAMES the page and the button"),
