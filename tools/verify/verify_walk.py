...
def hold(res, label, tools_expected, allow_unreachable=False):
    ...
    ck(res["protocolVersion"] == "1.11", "%s: protocol 1.11" % label)
...
if os.path.isfile(led):
    ...
    for pid, want in (("csrbt-organism", 35), ("csrbt-lab", 9), ("csrbt-page", 22),
                      ("csrbt-organism@mcp", 35), ("csrbt-lab@mcp", 9), ("csrbt-page@mcp", 22)):
        e = L.get(pid) or {}
...
if os.path.isfile(led):
    ...
    ck(len(driven_somewhere) == 22 and all(v > 0 for v in driven_somewhere.values()),
       "every one of the 22 page tools was driven on at least one page: never %s"
