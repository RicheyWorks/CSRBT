# -*- coding: utf-8 -*-
"""Inline the outbox into the pages that hold data worth losing.

Same constraint as FEK and Keep: no build step, so the shared code is inlined
rather than linked, and inlined copies drift unless something regenerates them.
...
_spec = importlib.util.spec_from_file_location("outbox", os.path.join(ROOT, "tools", "outbox.py"))
outbox = importlib.util.module_from_spec(_spec); _spec.loader.exec_module(outbox)
...
BANNER = "/* ============ Outbox (what has not left this device) v"
CSS_OPEN = re.compile(r"[ \t]*/\* =+ Outbox \(what has not left this device\) v[\d.]+ =+")


def css_span(src, from_=0):
    """Banner to the end of outbox.CSS's own last rule.
    ...
    tail = [l for l in outbox.CSS.strip("\n").split("\n") if l.strip()][-1].strip()
...
def dedupe_css(src):
    ...
    to carry the Outbox stylesheet four times with --check reporting the tree
    clean.
    """
...
def main(argv):
    ...
    for name in CONSUMERS:
        ...
        span = css_span(out)
        if span:
            out = out[:span[0]] + outbox.CSS.strip("\n") + out[span[1]:]
        else:
            i = out.rfind("</style>")
            if i == -1: missing.append((name, "no </style>")); continue
            out = out[:i] + outbox.CSS.strip("\n") + "\n" + out[i:]
        if JS_RE.search(out):
            out = JS_RE.sub(lambda m: outbox.JS.strip("\n"), out, count=1)
        else:
            missing.append((name, "no OUT block -- wire the page first")); continue
        if out != src:
            changed.append(name)
            if not check:
                io.open(path, "w", encoding="utf-8").write(out)
    print("Outbox %s" % outbox.VERSION)
    print("-" * 56)
    for n in changed:
