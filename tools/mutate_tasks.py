...
MUTANTS = [
    ...
    ("$$ is stripped to nothing, so the figure loses its sign",
     ...
     '    if isinstance(value, str) and value.startswith("$$"):\n        return value[2:]',
     "one literal dollar sign"),
    # ---- what the page said, claimed on the act (ADR-200) ----
    ("any-contains reads the wrong way round",
     '                    hit = any(isinstance(x, str) and val in x for x in got)',
     '                    hit = any(isinstance(x, str) and x in val for x in got)',
     "any-contains CONFIRMS when SOME message holds the text"),
    ("any-contains demands the whole message, so a refusal with a tail is missed",
     '                    hit = any(isinstance(x, str) and val in x for x in got)',
     '                    hit = any(isinstance(x, str) and x == val for x in got)',
     "any-contains CONFIRMS when SOME message holds the text"),
    ("only the FIRST message is looked at, so a refusal raised second is lost",
     '                    hit = any(isinstance(x, str) and val in x for x in got)',
     '                    hit = bool(got) and isinstance(got[0], str) and val in got[0]',
     "any-contains CONFIRMS when SOME message holds the text"),
    ("the mirror is not a mirror: none-contains answers the same as any-contains",
     '                    ok = hit if op == "any-contains" else not hit',
     '                    ok = hit',
     "none-contains is the mirror"),
    ("a path that is not a list is answered about anyway",
     '                if not isinstance(got, (list, tuple)) or not isinstance(val, str):\n                    ok = (op == "none-contains") and isinstance(got, (list, tuple))',
     '                if not isinstance(got, (list, tuple)) or not isinstance(val, str):\n                    ok = (op == "any-contains") and isinstance(got, str) and val in got',
     "BOTH DIRECTIONS ARE FALSE ON A PATH THAT IS NOT A LIST"),
    ("a missing path proves silence",
     '            elif not present:\n                ok = False',
     '            elif not present:\n                ok = (op == "none-contains")',
     "a path that is not in the response does not prove silence"),
    ("contains on a list is quietly widened to a substring search",
     '                    ok = val in got if isinstance(got, (list, str, dict)) else False',
     '                    ok = (any(isinstance(x, str) and val in x for x in got)\n                          if isinstance(got, (list, tuple))\n                          else (val in got if isinstance(got, (str, dict)) else False))',
     "CONTAINS ON A LIST IS STILL EXACT MEMBERSHIP"),
]
