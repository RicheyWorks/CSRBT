...
the report (`read-report`, note its `output.stamp`), then do the batch's acts,
and guard every act you expect to change a figure with `if_stamp: <that report
stamp>` — after the first such act moves the figures the next guarded act will
be refused `stale`; read the report again (`since` the old stamp shows what
moved) and continue with the new stamp. That refusal is the door working, not
an error: it tells you the figures moved since you last read them. When the
brief's RUNGS line names DESTRUCTIVE, the goal includes an irreversible step
(an Undo, a Clear, a remove) — do it, and guard it with a SNAPSHOT stamp from
the read you based it on. Stop when the goal is met. Keep `--trace` on for
every call.
