...
MUTANTS = [
    ...
    ("a drive-qualified variable is never a fault",
     "            if m.group(1).lower() not in _PS_SCOPES:",
     "            if False:",
     "IS A DRIVE-QUALIFIED VARIABLE TO POWERSHELL"),
    ("every variable followed by a colon is a fault, the scopes included",
     "            if m.group(1).lower() not in _PS_SCOPES:",
     "            if True:",
     "a scope qualifier is not a fault"),
    ("only the generated scripts are read for parse faults",
     '    for sp in sorted(glob.glob(os.path.join(PUSH, "*.ps1"))):',
     '    for sp in sorted(glob.glob(os.path.join(PUSH, "push-adr[0-9]*[0-9].ps1"))):',
     "the hand-written ones included"),
